function EstOut = estimate_garch_aux(R, varargin)
%ESTIMATE_GARCH Rolling-window estimation of a univariate GARCH(1,1)
% model for K assets.
%
%   EstOut = ESTIMATE_GARCH(R) estimates a GARCH(1,1) model with normal
...
switch dist
    case 'norm'
        ...
        start      = [   0.2,    0.1,    0.8];
        ll_fun     = @(pars, r) VarianceModels.univ_garch(pars, r);
        get_nu     = false;
        get_lambda = false;
        idx_nu     = NaN;
        idx_lambda = NaN;        
    ...
    case 't'
        ...
        ll_fun     = @(pars, r) VarianceModels.univ_garch_t(pars, r);
        get_nu     = true;
        get_lambda = false;
    ...
    case 'skewt'
        ...
        ll_fun     = @(pars, r) VarianceModels.univ_garch_skew_t(pars, r);
        get_nu     = true;
        get_lambda = true;
    ...
    case 'laplace'
        ...
        ub         = [   Inf; 1-epsi; 1-epsi];
        start      = [   0.2,    0.1,    0.8];
        ll_fun     = @(pars, r) VarianceModels.univ_garch_laplace(pars, r);
        get_nu     = false;
        get_lambda = false;
        idx_nu     = NaN;
        idx_lambda = NaN;        
...
GARCHpars_reest = NaN(n_reest, 3, K);
